...
void SetIntersection(std::vector<int> &v1, std::vector<int> &v2, std::vector<int> &intersection)
{
  std::sort(v1.begin(), v1.end());
  std::sort(v2.begin(), v2.end());
  intersection.clear();
...
std::vector<int> find_path_bfs(
    int start,
    int goal,
    const Eigen::MatrixXi &adj_matrix)
{
  int n = adj_matrix.rows();

  if (start < 0 || start >= n || goal < 0 || goal >= n)
  {
    std::cerr << "[find_path_bfs] Invalid start or goal index: "
              << "start = " << start << ", goal = " << goal << ", size = " << n << std::endl;
    return {};
  }

  if (adj_matrix.rows() != adj_matrix.cols())
  {
    std::cerr << "[find_path_bfs] Error: adj_matrix is not square!" << std::endl;
    return {};
  }
  ...
  while (!q.empty())
  {
    ...
    {
      if (curr < 0 || curr >= n || neighbor < 0 || neighbor >= n)
        continue;

      if (adj_matrix(curr, neighbor) != 0 && !visited[neighbor])
      {
  ...
  if (!visited[goal])
  {
    std::cerr << "[find_path_bfs] No path found from " << start << " to " << goal << std::endl;
    return {};
  }
  ...
  while (at != start)
  {
    ...
    {
      std::cerr << "[find_path_bfs] Error: broken path during backtracking at " << at << std::endl;
      return {};
    }
    at = parent[at];
  }
...
Eigen::Vector3d HSVtoRGB(double h, double s, double v)
{
  ...
  return Eigen::Vector3d(b, g, r); // 注意：OpenCV 中 BGR 顺序
}

// 主函数：ID 映射为鲜艳 RGB 颜色
Eigen::Vector3d idToColor(int id)
{
  if (id == 0)
  {
    return Eigen::Vector3d(255, 255, 255); // 白色表示无效或未分配的 ID
  }

  double hue = (id * 57) % 360; // 色相：分布在 0~360
  double saturation = 0.85;     // 高饱和度
  double value = 0.95;          // 高亮度

  return HSVtoRGB(hue, saturation, value);
}

// 将点坐标转换为体素网格索引
Eigen::Vector3i point_to_voxel(
    const Eigen::Vector3f &pt,
    const Eigen::Vector3f &origin_shift, // e.g., (-150.0, -150.0, -1.0)
    float inv_resolution)                // precomputed 1.0 / grid_resolution
{
    ...
    float inv_resolution)                // precomputed 1.0 / grid_resolution
{
  ...
  Eigen::Vector3f shifted = pt_eigen * inv_resolution + origin_shift;
  return shifted.array().floor().cast<int>();
}

// 将体素网格索引转换为点坐标
Eigen::Vector3f voxel_to_point(
...
Eigen::Vector3i point_to_voxel_cropped(
    const Eigen::Vector3f &pt,
    const Eigen::Vector3f &origin_shift, // e.g., (-150.0, -150.0, -1.0)
    float inv_resolution,
    std::vector<Eigen::Vector2i> &bbox) // crop region bbox
{
  Eigen::Vector3f shifted = pt * inv_resolution + origin_shift;
...
// 将体素网格索引转换为点坐标
Eigen::Vector3f voxel_to_point_cropped(
    const Eigen::Vector3i &voxel_index,
    const Eigen::Vector3f &origin_shift, // e.g., (-150.0, -150.0, -1.0)
    float resolution,
    std::vector<Eigen::Vector2i> &bbox) // crop region bbox
