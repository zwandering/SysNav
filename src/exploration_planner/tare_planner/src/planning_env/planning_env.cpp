...
void PlanningEnvParameters::ReadParameters(rclcpp::Node::SharedPtr nh)
{
  ...
  nh->get_parameter("kDoorCollisionCheckRadius", kDoorCollisionCheckRadius);
  nh->get_parameter("kDoorCollisionCheckPointNumThr", kDoorCollisionCheckPointNumThr);
  ...
  , robot_position_update_(false)
{
  ...
  occupied_cloud = std::make_shared<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
      nh, "occupied_cloud", world_frame_id);
}
...
bool PlanningEnv::CheckLineOfSightInOccupancyGrid(const Eigen::Vector3i& start_pos, 
                                                  const Eigen::Vector3i& end_pos) {
  if (!rolling_occupancy_grid_) {
    RCLCPP_WARN(rclcpp::get_logger("standalone_logger"), 
                "Rolling occupancy grid not initialized");
    return false;
  }
  
  return rolling_occupancy_grid_->CheckLineOfSight(start_pos, end_pos);
}

Eigen::Vector3i PlanningEnv::Pos2Sub(const Eigen::Vector3d& pos) const
{
  return rolling_occupancy_grid_->Pos2Sub(pos);
}

bool PlanningEnv::InRange(const Eigen::Vector3i& voxel_index) const
{
  return rolling_occupancy_grid_->InRange(voxel_index);
}
...
void PlanningEnv::UpdateFrontiers()
{
  if (parameters_.kUseFrontier)
  {
    ...
    if (!frontier_cloud_->cloud_->points.empty())
    {
      ...
      {
        if (coverage_boundary_.points.empty())
        {
          RCLCPP_WARN(
              rclcpp::get_logger("standalone_logger"),
              "PlanningEnv::UpdateFrontiers(): coverage boundary is empty but boundary mode is enabled! ");
        }
        else
        {
...
bool PlanningEnv::DoorInCollision(double x, double y, double z) const
{
  if (stacked_cloud_->cloud_->points.empty())
  {
    RCLCPP_WARN(rclcpp::get_logger("standalone_logger"),
  ...
  std::vector<int> neighbor_indices;
  std::vector<float> neighbor_sqdist;
  stacked_vertical_surface_cloud_kdtree_->radiusSearch(check_point, parameters_.kDoorCollisionCheckRadius,
                                                       neighbor_indices, neighbor_sqdist);
  if (neighbor_indices.size() > parameters_.kDoorCollisionCheckPointNumThr)
  {
    return true;
  }
  else
                                   ...
                                   int& uncovered_point_num, int& uncovered_frontier_point_num)
{
  ...
  {
    PlannerCloudPointType point = planner_cloud_->cloud_->points[i];
    // if the point is already covered, skip it
    if (point.g > 0)
    {
      continue;
    }
