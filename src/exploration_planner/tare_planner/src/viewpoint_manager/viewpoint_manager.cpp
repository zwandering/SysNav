...
ViewPointManager::ViewPointManager(rclcpp::Node::SharedPtr nh) : initialized_(false)
{
  ...
  current_room_id_ = -1; // -1 means no room is selected
  transit_across_room_ = false; // used to check if the robot is allowed to transit across rooms
  enter_wrong_room_ = false; // used to check if the robot accidentally enters a new room
}
...
void ViewPointManager::GetVisualizationCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr& vis_cloud)
{
  ...
  for (int i = 0; i < vp_.kViewPointNumber; i++)
  {
    if (IsViewPointCandidate(i, true))
    {
      ...
      else if (ViewPointRoomCandidate(i, true))
      {
        vis_point.intensity = -1.0;
      }
      else
...
void ViewPointManager::GetFreespaceCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &freespace_cloud)
{
  freespace_cloud->clear();
  for (int i = 0; i < vp_.kViewPointNumber; i++)
  {
    if (!ViewPointInCollision(i) && ViewPointInLineOfSight(i) && ViewPointConnected(i))
    {
      ...
      pcl::PointXYZI free_point;
      free_point.x = position.x;
      free_point.y = position.y;
      free_point.z = position.z;
      freespace_cloud->points.push_back(free_point);
    }
  }
}
...
void ViewPointManager::CheckViewPointRoomBoundaryCollision()
{ 
  // Check for the polygon boundary and nogo zones
  for (int i = 0; i < vp_.kViewPointNumber; i++)
  {
    geometry_msgs::msg::Point viewpoint_position = GetViewPointPosition(i, true);
    Eigen::Vector3f viewpoint_position_f(viewpoint_position.x, viewpoint_position.y, viewpoint_position.z);
    Eigen::Vector3i viewpoint_position_voxel = misc_utils_ns::point_to_voxel(viewpoint_position_f, vp_.shift_, 1.0/vp_.room_resolution_);
    // check if the voxel index is valid for the room mask
    if (viewpoint_position_voxel.x() < 0 || viewpoint_position_voxel.x() >= room_mask_.cols ||
        viewpoint_position_voxel.y() < 0 || viewpoint_position_voxel.y() >= room_mask_.rows)
    {
      SetViewPointInRoom(i, false, true);
      continue;
    }
    ...
    // if (room_id == current_room_id_ || current_room_id_ == -1)
    {
      ...
      //              "Viewpoint %d with room id %d is in room %d",
      //              i, room_id, current_room_id_);
      SetViewPointInRoom(i, true, true);
    }
    else
    {
      SetViewPointInRoom(i, false, true);
...
// Room
bool ViewPointManager::ViewPointInRoom(int viewpoint_ind, bool use_array_ind)
{
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].InRoom();
}
void ViewPointManager::SetViewPointInRoom(int viewpoint_ind, bool in_room, bool use_array_ind)
{
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetInRoom(in_room);
}
// Room candidate viewpoint
bool ViewPointManager::ViewPointRoomCandidate(int viewpoint_ind, bool use_array_ind)
{
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  return viewpoints_[array_ind].RoomCandidate();
}
void ViewPointManager::SetViewPointRoomCandidate(int viewpoint_ind, bool room_candidate, bool use_array_ind)
{
  int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
  viewpoints_[array_ind].SetRoomCandidate(room_candidate);
}
// Line of Sight
...
bool ViewPointManager::InLocalPlanningHorizonWithoutRoom(const Eigen::Vector3d& position)
{
  int viewpoint_ind = GetViewPointInd(position);
  if (InRange(viewpoint_ind))
  {
