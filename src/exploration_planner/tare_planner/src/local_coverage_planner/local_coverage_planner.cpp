...
namespace local_coverage_planner_ns
{
  ...
  LocalCoveragePlanner::LocalCoveragePlanner(rclcpp::Node::SharedPtr nh)
      : lookahead_point_update_(false), use_frontier_(true),
        local_coverage_complete_(false), transit_across_room_(false), enter_wrong_room_(false), object_found_(false), anchor_object_found_(false)
  {
              ...
              global_path.nodes_.front().position_))
      {
        for (int i = 0; i < global_path.nodes_.size(); i++)
        {
          ...
          if (global_path.nodes_[i].type_ ==
              ...
              !viewpoint_manager_->InLocalPlanningHorizonWithoutRoom(
                  global_path.nodes_[i].position_))
          {
            break;
          }
      ...
      std::vector<int> &navigation_viewpoint_indices)
  {
    ...
    if (!lookahead_point_update_ ||
        ...
        transit_across_room_ || object_found_)
    {
  ...
  void LocalCoveragePlanner::GetRoomCandidateViewPointIndices(
    std::vector<int> &viewpoint_roomcandidate_indices)
  {
    viewpoint_roomcandidate_indices.clear();
    for (int i = 0; i < viewpoint_manager_->GetViewPointNum(); i++)
    {
      if (viewpoint_manager_->ViewPointRoomCandidate(i))
      {
        viewpoint_roomcandidate_indices.push_back(i);
      }
    }
    // if (viewpoint_roomcandidate_indices.empty())
    // {
    //   RCLCPP_WARN(rclcpp::get_logger("tare_planner"),
    //               "No room candidate viewpoints found");
    // }
  }
      ...
      int uncovered_point_num, int uncovered_frontier_point_num)
  {
    ...
    GetRoomCandidateViewPointIndices(viewpoint_roomcandidate_indices);
    ...
    for (auto &viewpoint_array_ind : last_selected_viewpoint_array_indices_)
    {
      ...
      if (viewpoint_manager_->ViewPointVisited(viewpoint_array_ind, true) ||
          !viewpoint_manager_->IsViewPointCandidate(viewpoint_array_ind, true) ||
          !viewpoint_manager_->ViewPointInRoom(viewpoint_array_ind, true))
      {
        continue;
      }
      int covered_point_num = viewpoint_manager_->GetViewPointCoveredPointNum(
    ...
    for (const auto &ind : viewpoint_roomcandidate_indices)
    {
      int array_ind = viewpoint_manager_->GetViewPointArrayInd(ind);
      pre_selected_viewpoint_array_indices.push_back(array_ind);
    }
    ...
    if (transit_across_room_ || object_found_ || anchor_object_found_)
    {
    ...
    else if (!queue.empty() && queue[0].first > parameters_.kMinAddPointNum)
    {
    ...
    last_selected_viewpoint_indices_.clear();
    for (const auto &ind : ordered_viewpoint_indices)
    {
      if (!viewpoint_manager_->ViewPointRoomCandidate(ind))
      {
        last_selected_viewpoint_indices_.push_back(ind);
      }
    }
      ...
      pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud)
  {
    ...
    std::vector<int> viewpoint_roomcandidate_indices;
    GetRoomCandidateViewPointIndices(viewpoint_roomcandidate_indices);
    for (const auto &viewpoint_index : viewpoint_roomcandidate_indices)
    {
      geometry_msgs::msg::Point position =
          viewpoint_manager_->GetViewPointPosition(viewpoint_index);
      pcl::PointXYZI point;
