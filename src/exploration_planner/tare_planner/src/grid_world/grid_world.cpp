    ...
    std::vector<int>& ordered_cell_indices, const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  std::vector<geometry_msgs::msg::Point> exploring_cell_positions_global;
  std::vector<int> exploring_cell_indices_global;
  for (int i = 0; i < subspaces_->GetCellNumber(); i++)
  {
    if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING)
    {
          ...
          (subspaces_->GetCell(i).GetViewPointIndices().empty() && subspaces_->GetCell(i).GetVisitCount() > 1))
      {
        ...
        else
        {
          ...
          bool in_room = false;
          Eigen::Vector3f connection_point_f(connection_point.x(), connection_point.y(), connection_point.z());
          Eigen::Vector3i connection_point_voxel = misc_utils_ns::point_to_voxel(connection_point_f, shift_, 1.0 / room_resolution_);
          int room_id = room_mask_.at<int>(connection_point_voxel.x(), connection_point_voxel.y());
          if (room_id == current_room_id_ || current_room_id_ == -1)
          {
            in_room = true;
          ...
          if (reachable)
          {
            exploring_cell_positions_global.push_back(connection_point_geo);
            exploring_cell_indices_global.push_back(i);
            if (in_room)
            {
  ...
  /****** Transit across room || Found target object ******/ 
  if (transit_across_room_ || object_found_)
  {
    nav_msgs::msg::Path across_room_path;
    if (transit_across_room_)
    {
      keypose_graph->GetShortestPath(global_path_robot_position, door_position_, true, across_room_path, true);
    }
    if (object_found_)
    {
      keypose_graph->GetShortestPath(global_path_robot_position, found_object_position_, true, across_room_path, true);
    }
    if (across_room_path.poses.size() >= 2)
    {
      global_path.FromPath(across_room_path);
      ...
      global_path.nodes_.back().type_ = exploration_path_ns::NodeType::DOOR;
      // Make it a loop
    ...
    else
    {
      ...
      //                     "GridWorld::SolveGlobalTSP: Cannot find path across room, size: "
      //                         << across_room_path.poses.size());
      // TODO: find a path
    }
    return global_path;
  }

  // ----------- Only for the dynamic mode --------------
  if (anchor_object_found_)
  {
    RCLCPP_INFO(rclcpp::get_logger("standalone_logger"), "GridWorld::SolveGlobalTSP: Found anchor object, solving TSP");
    // Put the current robot position in the end
    found_anchor_object_position_.push_back(global_path_robot_position);

    /******* Construct the distance matrix *****/
    std::vector<std::vector<int>> distance_matrix(found_anchor_object_position_.size(),
                                                  std::vector<int>(found_anchor_object_position_.size(), 0));
    for (int i = 0; i < found_anchor_object_position_.size(); i++)
    {
      for (int j = 0; j < i; j++)
      {
        if (!use_keypose_graph_ || keypose_graph == nullptr || keypose_graph->GetNodeNum() == 0)
        {
          ...
          distance_matrix[i][j] =
              static_cast<int>(10 * misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(
                                        found_anchor_object_position_[i], found_anchor_object_position_[j]));
        }
        else
        {
          // Use keypose graph
          ...
          distance_matrix[i][j] =
              static_cast<int>(10 * keypose_graph->GetShortestPath(found_anchor_object_position_[i],
                                                                   found_anchor_object_position_[j], false, path_tmp, false));
        }
      }
    }
    ...
    for (int i = 0; i < found_anchor_object_position_.size(); i++)
    {
      for (int j = i + 1; j < found_anchor_object_position_.size(); j++)
      {
    ...
    data_model.depot = found_anchor_object_position_.size() - 1;
    ...
    if (!use_keypose_graph_ || keypose_graph == nullptr || keypose_graph->GetNodeNum() == 0)
    {
      for (int i = 0; i < node_index.size(); i++)
      {
        ...
        pose.pose.position = found_anchor_object_position_[cell_ind];
        exploration_path_ns::Node node(found_anchor_object_position_[cell_ind],
    ...
    else
    {
      ...
      for (int i = 0; i < node_index.size() - 1; i++)
      {
        ...
        cur_position = found_anchor_object_position_[cur_ind];
        next_position = found_anchor_object_position_[next_ind];
  ...
  /****** When the room is near finish, set the near finish flag true ******/
  if (exploring_cell_indices.size() <= 1)
  {
    room_near_finished_ = true;
  }
  else
  {
    room_near_finished_ = false;
  ...
  /****** If no exploring cells in the current room, set the room finished flag ******/
  if (exploring_cell_indices.empty())
  {
    room_finished_ = true;
    
    // TODO: Need to be modified
    nav_msgs::msg::Path room_finished_path;
    geometry_msgs::msg::Point home_position = keypose_graph->GetFirstKeyposePosition();
    keypose_graph->GetShortestPath(global_path_robot_position, global_path_robot_position, true, room_finished_path, false);
    if (room_finished_path.poses.size() >= 2)
    {
      global_path.FromPath(room_finished_path);
      global_path.nodes_.front().type_ = exploration_path_ns::NodeType::ROBOT;
  ...
  /****** Return home ******/
  // if (exploring_cell_indices.empty())
  if (exploring_cell_indices_global.empty())
  {
    return_home_ = true;
