...
public:
  ...
  void CheckViewPointRoomBoundaryCollision();
  ...
  bool ViewPointInRoom(int viewpoint_ind, bool use_array_ind = false);
  void SetViewPointInRoom(int viewpoint_ind, bool in_room, bool use_array_ind = false);

  bool ViewPointRoomCandidate(int viewpoint_ind, bool use_array_ind = false);
  void SetViewPointRoomCandidate(int viewpoint_ind, bool in_room, bool use_array_ind = false);
  ...
  bool InLocalPlanningHorizonWithoutRoom(const Eigen::Vector3d& position);
  ...
  void GetFreespaceCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr &freespace_cloud);
  ...
  void SetEnterWrongRoom(bool enter_wrong_room)
  {
    enter_wrong_room_ = enter_wrong_room;
  }
...
private:
  ...
  bool enter_wrong_room_; // used to check if the robot accidentally enters a new room
};
