...
public:
  ...
  bool IsRoomFinished()
  {
    return room_finished_;
  }
  bool IsRoomNearFinished()
  {
    return room_near_finished_;
  }
  ...
  void SetTransitAcrossRoom(bool transit_across_room)
  {
    transit_across_room_ = transit_across_room;
  }
  void SetRoomPosition(const Eigen::Vector3d& room_position)
  {
    // transferring from Eigen::Vector3d to geometry_msgs::msg::Point
    door_position_.x = room_position.x();
    door_position_.y = room_position.y();
    door_position_.z = room_position.z();
  }
  void SetRoomMask(const cv::Mat &room_mask)
  {
    room_mask_ = room_mask;
  }
  void SetCurrentRoomId(int room_id)
  {
    current_room_id_ = room_id;
  }
  void SetObjectFound(bool object_found)
  {
    object_found_ = object_found;
  }
  void SetAnchorObjectFound(bool anchor_object_found)
  {
    anchor_object_found_ = anchor_object_found;
  }
  void SetFoundObjectPosition(const geometry_msgs::msg::Point& object_position)
  {
    found_object_position_.x = object_position.x;
    found_object_position_.y = object_position.y;
    found_object_position_.z = object_position.z;
  }
  void SetFoundAnchorObjectPosition(const std::vector<geometry_msgs::msg::Point>& viewpoint_positions)
  {
    found_anchor_object_position_.clear();
    for (const auto& position : viewpoint_positions)
    {
      geometry_msgs::msg::Point point;
      point.x = position.x;
      point.y = position.y;
      point.z = position.z;
      found_anchor_object_position_.push_back(point);
    }
  }
...
private:
  ...
  geometry_msgs::msg::Point door_position_;
  cv::Mat room_mask_;
  ...
  Eigen::Vector3f shift_; // shift for the room mask, e.g., (-150.0, -150.0, -1.0)
  bool room_finished_;
  bool room_near_finished_;

  bool object_found_;
  bool anchor_object_found_;
  geometry_msgs::msg::Point found_object_position_;
  std::vector<geometry_msgs::msg::Point> found_anchor_object_position_;
};
}  // namespace grid_world_ns
