...
struct planning_env_ns::PlanningEnvParameters
{
  ...
  double kDoorCollisionCheckRadius;
  int kDoorCollisionCheckPointNumThr;
...
public:
  ...
  void UpdateRegisteredCloud(typename pcl::PointCloud<PCLPointType>::Ptr& cloud, const int &registered_cloud_count_)
  {
  ...
  void UpdateKeyposeCloud(typename pcl::PointCloud<PCLPointType>::Ptr& keypose_cloud)
  {
    ...
    else
    {
      ...
      if (parameters_.kUseCoverageBoundaryOnObjectSurface)
      {
        if (coverage_boundary_.points.empty())
        {
          RCLCPP_WARN(
              ...
              "PlanningEnv::UpdateKeyposeCloud(): coverage boundary is empty but boundary mode is enabled! ");
        }
        else
        {
      ...
      // Get the diff cloud (并不是完全准确的diff_cloud,重叠区域仍然有点会被认为是diff)
      ...
      *vertical_surface_cloud_stack_[keypose_cloud_count_] = *(vertical_surface_cloud_->cloud_);
      ...
      stacked_vertical_surface_cloud_->Publish();
  ...
  void GetObsVoxelNumber(int &occupied_voxel_num)
  {
    occupied_voxel_num = 0;
    rolling_occupancy_grid_->GetObsVoxelNumber(occupied_voxel_num);
  }
  void GetCurrentObsVoxelInds(std::vector<int>& obs_curr){
    obs_curr.clear();
    rolling_occupancy_grid_->GetCurrentObsVoxelInds(obs_curr);
  }
  void GetUpdatedVoxelInds(std::vector<int> &updated_subs)
  {
    updated_subs.clear();
    rolling_occupancy_grid_->GetUpdatedVoxelInds(updated_subs);
  }
  void UpdateCoveredVoxels(const Eigen::Vector3d &origin)
  {
    rolling_occupancy_grid_->UpdateCoveredVoxels(origin);
  }
  pcl::PointCloud<pcl::PointXYZI>::Ptr GetUpdatedCloudInRange()
  {
    return rolling_occupancy_grid_->GetUpdatedCloudInRange();
  }
  bool CheckLineOfSightInOccupancyGrid(const Eigen::Vector3i& start_pos, const Eigen::Vector3i& end_pos);
  Eigen::Vector3i Pos2Sub(const Eigen::Vector3d &pos) const;
  bool InRange(const Eigen::Vector3i &voxel_index) const;

  bool DoorInCollision(double x, double y, double z) const;
...
private:
  ...
  std::shared_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> occupied_cloud;
};
