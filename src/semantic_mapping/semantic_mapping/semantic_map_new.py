...
class ObjMapper():
    def __init__(
        ...
        self.deleted_objects_batch = []  # Batch collect deleted objects
    ...
    def update_map(self, detections, detection_stamp, detection_odom, cloud, image=None, viewpoint_stamp_to_process=None):
        ...
        masks = [mask.copy()  for mask, confidence in zip(detections['masks'], detections['confidences']) if confidence >= self.confidence_thres]
        labels = [label.copy() for label, confidence in zip(detections['labels'], detections['confidences']) if confidence >= self.confidence_thres]
        obj_ids = [obj_id.copy() for obj_id, confidence in zip(detections['ids'], detections['confidences']) if confidence >= self.confidence_thres]
        bboxes = [bbox.copy() for bbox, confidence in zip(detections['bboxes'], detections['confidences']) if confidence >= self.confidence_thres]
        ...
        # Flush batch deleted objects at the end of update_map
        self.flush_deleted_objects_batch(detection_stamp)

        self.valid_cnt += 1


    def publish_deleted_object(self, single_obj, detection_stamp):
        """Collect deleted object for batch publishing instead of immediate publish"""
        ...
        # Add to batch list instead of publishing immediately
        self.deleted_objects_batch.append(delete_msg)
    ...
    def flush_deleted_objects_batch(self, stamp):
        """Publish all deleted objects as a batch"""
        if self.object_node_pub is None or len(self.deleted_objects_batch) == 0:
            return
        
        seconds = int(stamp)
        nanoseconds = int((stamp - seconds) * 1e9)
        
        # Create and publish ObjectNodeList with deleted objects
        deleted_list_msg = ObjectNodeList()
        deleted_list_msg.header.stamp = Time(seconds=seconds, nanoseconds=nanoseconds).to_msg()
        deleted_list_msg.header.frame_id = 'map'
        deleted_list_msg.nodes = self.deleted_objects_batch
        
        self.object_node_pub.publish(deleted_list_msg)
        self.log_info(f"🗑️ Batch deleted {len(self.deleted_objects_batch)} objects")
        
        # Clear the batch
        self.deleted_objects_batch = []
    ...
    def to_ros2_msgs(self, stamp, viewpoint_id):
        ...
        bbox_msg_list = []
        text_msg_list = []
        object_nodes_list = []  # Collect all ObjectNode messages
        ...
        for single_obj in self.single_obj_list:
            ...
            if label == self.target_object and not single_obj.is_asked_vlm:
                label_with_id = f"Potential Target({id})"
            else:
                label_with_id = f"{single_obj.get_dominant_label()}({id})"
            ...
            # Add to batch list instead of publishing immediately
            object_nodes_list.append(object_node_msg)
        ...
        # Publish all objects as a batch
        if self.object_node_pub is not None and len(object_nodes_list) > 0:
            object_node_list_msg = ObjectNodeList()
            object_node_list_msg.header.stamp = Time(seconds=seconds, nanoseconds=nanoseconds).to_msg()
            object_node_list_msg.header.frame_id = 'map'
            object_node_list_msg.nodes = object_nodes_list
            self.object_node_pub.publish(object_node_list_msg)
