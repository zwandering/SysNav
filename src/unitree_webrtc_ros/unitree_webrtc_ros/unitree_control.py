...
class UnitreeControlNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('control_mode', 'wireless_controller')
        ...
        self.exit_obstacle_avoidance_srv = self.create_service(Trigger, 'exit_obstacle_avoidance', self.exit_obstacle_avoidance_callback)

        self._execute_obstacle_avoidance_command('ExitObstacleAvoidance', parameter={"enable": False})
    ...
    def _execute_obstacle_avoidance_command(self, command_name: str, parameter: dict = None) -> Trigger.Response:
        """Execute an obstacle avoidance command."""
        response = Trigger.Response()
        ...
        try:
            request_data = {"api_id": 1001,
                            "parameter": parameter if parameter else {'enable': False}}
            # if parameter:
            #     request_data["parameter"] = parameter
    ...
    def exit_obstacle_avoidance_callback(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        """Service callback to exit obstacle avoidance mode."""
        return self._execute_obstacle_avoidance_command('ExitObstacleAvoidance', parameter={"enable": False})
    
    def start_obstacle_avoidance_callback(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        """Service callback to start obstacle avoidance mode."""
        return self._execute_obstacle_avoidance_command('StartObstacleAvoidance', parameter={"enable": True})
    ...
    def joystick_callback(self, msg: Joy):
        ...
        # Example: Log joystick axes and buttons
        self.log_info(f'Joystick axes: {msg.axes}, buttons: {msg.buttons}')
        # A start obstacle avoidance, B exit obstacle avoidance
        # X stand up, Y lie down
        if msg.buttons[1] == 1:  # B button
            self.exit_obstacle_avoidance_callback(None, None)
        if msg.buttons[0] == 1:  # A button
            self.start_obstacle_avoidance_callback(None, None)
        if msg.buttons[2] == 1:  # X button
            self.standup_callback(None, None)
        if msg.buttons[3] == 1:  # Y button
            self.liedown_callback(None, None)
